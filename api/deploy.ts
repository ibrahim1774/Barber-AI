...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Create repository name from shop name
    const repoName = websiteData.shopName
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-|-$/g, '');

    const fullRepoName = `${repoName}-barber-site`;

    // Step 1: Create GitHub repository
    console.log('Creating GitHub repository...');
    const { data: repo } = await octokit.repos.createForAuthenticatedUser({
      name: fullRepoName,
      description: `Website for ${websiteData.shopName} - Generated by Prime Barber AI`,
      private: false,
      auto_init: true,
    });
